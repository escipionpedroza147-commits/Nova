...
export const useStore = create((set, get) => ({
  ...
  settings: { restoreTabs: true, showShortcuts: true, sidebarHidden: false },
  ...
  async boot() {
    ...
    if (parsed.settings && typeof parsed.settings === 'object') patch.settings = { ...get().settings, ...parsed.settings };
  ...
  // ---------- settings / ui ----------
  patchSettings(patch) { set((s) => ({ settings: { ...s.settings, ...patch } })); get().scheduleSave(); },
