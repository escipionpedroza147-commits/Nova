...
export default function AiPage() {
  ...
  useEffect(() => {
    const applySeed = () => {
      if (!window.__corvAskSeed) return;
      const seed = window.__corvAskSeed; window.__corvAskSeed = null;
      if (seed === 'Summarize this page for me.') { setPrompt(''); sendAiMessage(seed); }
      else { setPrompt(`About this: “${seed}” — `); composerRef.current?.focus(); }
    };
    applySeed();
    window.addEventListener('corv-ask-seed', applySeed);
    return () => window.removeEventListener('corv-ask-seed', applySeed);
  }, []);
  useEffect(() => {
