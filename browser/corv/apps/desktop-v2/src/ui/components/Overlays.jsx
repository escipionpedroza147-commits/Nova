...
function AiSettings() {
  ...
  React.useEffect(() => {
    window.vcorv?.aiConfigGet?.().then((result) => {
      if (!result) return;
      setConfig(result);
      setModel(result.model || '');
      setProvider(result.provider || 'openrouter');
    }).catch(() => {});
  }, []);
  ...
  const save = async () => {
    ...
    const ok = await window.vcorv?.aiConfigSet?.(patch);
    if (ok) {
      store().showToast('AI settings saved');
      setKey('');
      const fresh = await window.vcorv?.aiConfigGet?.();
      setConfig(fresh);
    } else store().showToast('Could not save AI settings');
  };

  const removeKey = async () => {
    await window.vcorv?.aiConfigSet?.({ clearKey: true });
    const fresh = await window.vcorv?.aiConfigGet?.();
    setConfig(fresh);
    store().showToast('API key removed');
  };

  return (
    <div className="ai-settings">
      <div className="ai-settings-title">Corv AI · bring your own key</div>
      <select value={provider} onChange={(event) => setProvider(event.target.value)} aria-label="AI provider">
        <option value="openrouter">OpenRouter</option>
        <option value="openai">OpenAI-compatible</option>
      </select>
      {provider === 'openai' && (
        <input type="text" placeholder="Endpoint (https://api.openai.com/v1)" defaultValue={config?.endpoint || ''} onBlur={(event) => window.vcorv?.aiConfigSet?.({ endpoint: event.target.value.trim() })} />
      )}
      <input type="password" placeholder={config?.hasKey ? `Key saved ${config.keyHint} — paste to replace` : 'Paste API key'} value={key} onChange={(event) => setKey(event.target.value)} aria-label="API key" />
      <input type="text" placeholder="Model (e.g. openai/gpt-4o-mini)" value={model} onChange={(event) => setModel(event.target.value)} aria-label="Model" />
      <div className="ai-settings-actions">
        <button type="button" className="panel-action" onClick={save}>Save</button>
        {config?.hasKey && <button type="button" className="panel-action danger" onClick={removeKey}>Remove key</button>}
      </div>
      <div className="ai-settings-note">Without a key, Corv uses local AI (Ollama) when available. Keys are stored only on this Mac.</div>
    </div>
  );
}
            ...
            {downloads.length ? downloads.slice().reverse().map((item) => {
              ...
              return (
                <div key={item.id} className="list-item download-item">
                  <button type="button" title="Open file" onClick={() => window.vcorv?.openDownload?.(item.savePath || '')}>
                    <strong>{item.filename || 'Download'}</strong>
                    <small>{item.state === 'progressing' ? `Downloading · ${percent}%` : item.state === 'completed' ? 'Completed' : item.state}</small>
                  </button>
                  <button type="button" className="download-reveal" title="Show in Finder" aria-label="Show in Finder" onClick={() => window.vcorv?.showDownload?.(item.savePath || '')}>⌕</button>
                </div>
              );
