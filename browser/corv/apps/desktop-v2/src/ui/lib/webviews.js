...
export function onWebviewContextMenu(handler) { contextMenuHandler = handler; }
...
function createView(tab) {
  ...
  view.addEventListener('new-window', (event) => { event.preventDefault?.(); if (safeHttpUrl(event.url)) store().createTab(event.url, { background: event.disposition === 'background-tab' }); });
  ...
  view.addEventListener('context-menu', (event) => {
    ...
    if (params.linkURL && safeHttpUrl(params.linkURL)) {
      items.push({ label: 'Open link in new tab', action: () => store().createTab(params.linkURL) });
      items.push({ label: 'Copy link address', action: () => navigator.clipboard.writeText(params.linkURL) });
    }
    if (params.srcURL && /^https?:/i.test(params.srcURL) && params.mediaType === 'image') {
      items.push({ label: 'Open image in new tab', action: () => store().createTab(params.srcURL) });
      items.push({ label: 'Copy image address', action: () => navigator.clipboard.writeText(params.srcURL) });
    }
    if (params.selectionText) {
      items.push({ label: 'Copy', action: () => view.copy() });
      items.push({ label: `Search Google for “${params.selectionText.slice(0, 24)}${params.selectionText.length > 24 ? '…' : ''}”`, action: () => store().createTab(`https://www.google.com/search?q=${encodeURIComponent(params.selectionText)}`) });
      items.push({ label: 'Ask Corv about this', action: () => { window.__corvAskSeed = params.selectionText.slice(0, 2000); store().setAiOpen(true); } });
    }
    if (params.isEditable) {
      items.push({ label: 'Cut', action: () => view.cut() });
      items.push({ label: 'Copy', action: () => view.copy() });
      items.push({ label: 'Paste', action: () => view.paste() });
    }
    if (items.length) items.push({ type: 'separator' });
    items.push({ label: 'Summarize this page', action: () => { window.__corvAskSeed = 'Summarize this page for me.'; store().setAiOpen(true); window.dispatchEvent(new CustomEvent('corv-ask-seed')); } });
    items.push({ label: 'Back', action: () => view.canGoBack?.() && view.goBack() });
    items.push({ label: 'Reload', action: () => view.reload() });
    items.push({ label: 'Inspect element', action: () => { view.inspectElement?.(params.x || 0, params.y || 0); } });
    const rect = view.getBoundingClientRect(); contextMenuHandler?.(rect.left + (params.x || 0), rect.top + (params.y || 0), items);
  });
